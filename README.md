...
`collect --wait DURATION` avoids shell loops that repeatedly invoke `collect` before a sender finishes. It makes an immediate authenticated receipt-list request, then polls every five seconds by default until a matching completion, timeout, permanent request error, or Ctrl+C. Use `--poll-interval` from `1s` through `5m`; waits are capped at `24h`. With repeated `--upload ID`, all named uploads must be completed before processing begins. Without explicit IDs, normal collection waits for any unacknowledged completion, while `--list --all` waits for any completed receipt. This is a bounded local wait, not email/push notification, server push, a created daemon, or cross-process state.

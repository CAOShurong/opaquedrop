...
- Before chunk download, collection exercises that same hard-link primitive with a synced same-directory probe and verifies both names identify the same file. Unsupported output filesystems fail early and the probe names are removed; a successful probe is not a promise that a network share remains available.
...
- A bounded recipient wait polls only the authenticated receipt list and does not fetch manifests or chunks until its completion condition is met. It sends the collect capability only to the configured server, stops on timeout/permanent error/cancellation, creates no persistent watcher, and is not an external notification guarantee.

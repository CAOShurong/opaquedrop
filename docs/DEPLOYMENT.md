...
### Waiting for a completed submission

Use `opaquedrop collect --key FILE --out DIR --wait 30m` when a recipient-side process should stay open until the next unacknowledged submission is complete. The command checks immediately, then polls the existing authenticated receipt-list endpoint every five seconds. `--poll-interval` accepts `1s` through `5m`; `--wait` is capped at `24h`. A timeout is a nonzero error rather than a successful empty collection, so automation cannot confuse "nothing arrived" with "a file was collected". Ctrl+C cancels the current request or poll delay.

With one or more `--upload UPLOAD_ID` flags, the wait ends only after every distinct named upload is present in the completed receipt list; explicitly named acknowledged uploads remain eligible for re-collection. Without IDs, ordinary collection waits for the first unacknowledged completion. `--list` uses the same rule, except `--list --all` considers any completed receipt. After the condition is met, the normal list/manifest or manifest/chunk/ack path runs exactly once over the returned list.

Each poll sends the existing collect capability only to the configured OpaqueDrop server in the `Authorization` header. It does not contact SMTP, a webhook, Gotify, ntfy, or another notification service, and it does not expose filenames until normal local inspection/decryption. The default costs at most 12 successful list requests per minute per waiting process; the one-second minimum permits up to 60. Read failures use the normal bounded GET retry policy, while a permanent authorization/protocol error ends the wait instead of looping. The binary creates no scheduler or persistent watcher.
